// game params
const computerWait = .5; // secs for cop waiting
const startOver = 3;
const FPS = 30; // frames
const gridSize = 5; // rows and cols
...
const playerColorLight = "lightblue";
const compColor = "red";
...
function drawGrid(){
    for(let i = 0; i < gridSize +1 ; i++){ // loop for rows
        for(let j =0; j < gridSize + 1; j++){ // loop for cols
...
function drawScores(){
    ...
    drawText("PLAYER",WIDTH *.15,MARGIN * .25,playerColor, "25px Arial");
    drawText("COMPUTER",WIDTH *.60,MARGIN * .25,compColor, "25px Arial");
    
    drawText(playerScore,WIDTH *.23,MARGIN * .60,playerColor, "50px Arial");
    drawText(compScore,WIDTH *.73,MARGIN * .60,compColor, "50px Arial");
    
    if(playerScore + compScore == gridSize * gridSize){
        if(playerScore > compScore){
            drawText("PLAYER WINS!",WIDTH *.40,MARGIN * .50,compColor, "15px Arial");
            clearInterval(myVar);
            
        }
        else if(playerScore < compScore){
            drawText("COMPUTER WINS!",WIDTH *.40,MARGIN * .50,compColor, "15px Arial");
            clearInterval(myVar);
...
function getMax(level){
  ...
  let count =0;
    for(let i = 0; i <squares.length; i++){
        ...
        for(let t = 0; t < 4;t++){ 
            tempSquares = copy();
            tempSquare =  [ tempSquares[i][j].sideBot,
    ...
    OUTER: for(let i = 0; i < rows; i++){
        for(let j = 0; j < cols; j++){
            if(squares[i][j].contains(x,y)){
                ...
                // determine nieghbor
                let row = i, col = j, highlight, nieghbor = true;
...
function Square(x,y,w, h){
    ...
    this.drawFill = function(){
        ...
        ctx.fillRect(this.left + STROKE,this.top + STROKE,this.w - STROKE*2,this.h-STROKE*2);
